...
def addtempo(speech):
    effects = [['tempo', str(random.choice([0.9,1.1,1.2]))]]
    signal_sox, sample_rate = torchaudio.sox_effects.apply_effects_tensor(speech.unsqueeze(0), 16000, effects, channels_first=True)
    return signal_sox[0]
...
def addnoise(speech,noise_list,snr_dur=[0,20],max_frames=200):
    ...
    noise = load_wav(random.choice(noise_list[noies_type]),max_frames=max_frames,train_mode=True)
...
def addreverberate(speech,noise_list,max_frames):
    ...
    rir_raw = load_wav(random.choice(noise_list['reverb']),max_frames=max_frames,train_mode=False)
